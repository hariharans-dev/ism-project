...
const validateRequestBody_delete_registered_ip = [
  (req, res, next) => {
    const numberOfFields = Object.keys(req.body).length;
    if (numberOfFields == 0) {
      return res.status(400).json({ message: "no feild given" });
...
const delete_registered_ip_middleware = [
  validateRequestBody_delete_registered_ip,
];
router.post(
  "/delete-registered-ip",
  delete_registered_ip_middleware,
  deleteregisteredip
);

const validateRequestBody_show_attendance = [];
const show_attendance_middleware = [validateRequestBody_show_attendance];
router.post("/show-iplogs", show_attendance_middleware, show_ip_log);
